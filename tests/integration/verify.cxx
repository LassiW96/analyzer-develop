...
UInt_t verify( const char* root_file, const char* ref_file = nullptr )
{
  ...
  if( run ) {
    ...
    if( const auto* pRunInfo = tstf.Get<RunInfo>("Run_Info") ) {
      Info(here, "Reference run info loaded OK");
      const auto& [num, date, nevt, vers, beamE, beamM, psfact] = *pRunInfo;

      //------ Compare metadata
      if( run->GetNumber() != num ) {
        Error(here, "Run number mismatch, got %u, expected %u",
              run->GetNumber(), num);
        SETBIT(err, 6);
      } else
        Info(here, "Run number matches = %u", num);

      TDatime rd = run->GetDate();
      char ref_date[26];
      date.AsString(ref_date);
      if( rd != date ) {
        char read_date[26];
        rd.AsString(read_date);
        Error(here, "Run date mismatch, got %s, expected %s", read_date, ref_date);
        SETBIT(err, 7);
      } else
        Info(here, "Run date matches = %s", ref_date);

      if( run->GetDataVersion() != vers ) {
        Error(here, "Data version mismatch, got %d, expected %d",
              run->GetDataVersion(), vers);
        SETBIT(err, 8);
      } else
        Info(here, "Run data version matches = %d", vers);

      if( run->GetNumAnalyzed() != nevt ) {
        Error(here, "Number of events mismatch, got %llu, expected %u",
              run->GetNumAnalyzed(), nevt);
        SETBIT(err, 9);
      } else
        Info(here, "Number of analyzed events matches = %u", nevt);

      if( par ) {
        if( TMath::Abs(par->GetBeamE() - beamE) > ref_eps ) {
          Error(here, "Run parameters: beam energy mismatch, "
                "got %lf, expected %lf", par->GetBeamE(), beamE);
          SETBIT(err, 10);
        } else
          Info(here, "Beam energy run parameter matches = %lf", beamE);

        if( TMath::Abs(par->GetBeamM() - beamM) > ref_eps ) {
          Error(here, "Run parameters: beam particle mass mismatch, "
                "got %lf, expected %lf", par->GetBeamM(), beamM);
          SETBIT(err, 11);
        } else
          Info(here, "Beam particle mass run parameter matches = %lf", beamM);

        const auto& ps = par->GetPrescales();
        constexpr auto n_ps = std::min(std::size(psfact), ps.size());
        // auto n_ps2 = ps.size();
        // if( n_ps != n_ps2 ) {
        //   Error(here, "Run parameters: number of prescale factors differs, "
        //         "got %lu, expected %lu", n_ps2, n_ps);
        //   SETBIT(err, 12);
        ...
        Error(here, "No run parameters in analyzed file. Skipping "
              "associated tests.");
      }
    } else {
    ...
    Error(here, "No run data at all in analyzed file. Skipping "
          "metadata tests.");
  }
  ...
  if( T ) {
    ...
    //------ Clean up, which is what a real analysis script would do
    delete evlen;
    delete s1lac3;
    delete s1rac3;
    delete s2yt3;
    ...
    Error(here, "No Tree T in replay output. Skipping comparison of "
          "reconstructed data. This is very bad. Wrong ROOT file?");
  }
